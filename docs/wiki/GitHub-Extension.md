...
Icons are stored in `packages/github-extension/icons/` in four sizes required by Chrome:

アイコンは `packages/github-extension/icons/` に Chrome が要求する4サイズで保存されています：

| File | Size | Usage |
|------|------|-------|
| `icon16.png` | 16×16 | Favicon / toolbar small |
...
## Manifest i18n | マニフェストの国際化

The `name` and `description` fields in `manifest.json` use Chrome's `__MSG_xxx__` syntax to support multiple locales:

`manifest.json` の `name` と `description` フィールドは、Chrome の `__MSG_xxx__` 構文を使用して複数ロケールをサポートします：
...
Locale files are in `packages/github-extension/_locales/<lang>/messages.json`.
Chrome automatically selects the locale matching the browser language.

ロケールファイルは `packages/github-extension/_locales/<lang>/messages.json` にあります。
Chromeはブラウザ言語に合わせたロケールを自動選択します。
